...
In this practical we will carry out a disease mapping study (without spatial structure) of COVID-19 mortality in England during March-July 2020, partitioned in lower tier local authorities (LTLAs). 
...
The data set includes the number of COVID-19 deaths and population counts for the 628 wards of Greater London, stratified by gender and age groups. 
...
+ and with `tmap` as follows (note that it takes a very long time to run):
```{r eval=FALSE}
tmap_mode("view")
tm_shape(England_SMR) + tm_polygons("SMR",palette="RdYlGn", style="cont", n=8) +
            tm_borders(lty="solid", col="black") +
            tm_style("natural") +
            tm_layout(title="Interactive Map of SMRs at LTLA level in England")
```
...
# Obtaining the posterior relative risks (RRs)

The RRs will be smoothed using the Poisson-logNormal model with a spatially structured+unstructured prior on the spatial units. 

* We first obtain the adjacency matrix as seen in the lecture this morning:

```{r spdep, echo=TRUE, include=TRUE,eval=TRUE}
nb2INLA("LTLA.graph",poly2nb(LTLA))
LTLA.adj <- paste(getwd(),"/LTLA.graph",sep="")
```

We then specify the model using a BYM2 prior and then run in in `INLA`  
```{r INLA_BYM2, eval=TRUE}
ID<- seq(1,317)
formula_BYM2 <- O ~ f(ID, model="bym2", graph=LTLA.adj,
        ...
        prior = "pc",
        param = c(0.5, 2 / 3))))	
mod_BYM2 <- inla(formula=formula_BYM2, family="poisson", data=England_SMR, E=E, control.compute=list(dic=TRUE, waic=TRUE))
```
...
mod_BYM2$dic$dic
```	
* Compare with the model without the spatial structured component 
```{r INLA_HET, eval=TRUE}
formula_HET <- O ~ f(ID, model="iid", 
                            hyper=list(prec = list(
        prior = "pc.prec",
        param = c(0.5 / 0.31, 0.01))))
mod_HET <- inla(formula=formula_HET, family="poisson", data=England_SMR, E=E, control.compute=list(dic=TRUE, waic=TRUE))
mod_HET$waic$waic
mod_HET$dic$dic
```
We can conclude that the model with the spatial structured and unstructured component has a better fit so we will focus on that in the maps#
...
```{r eval=TRUE}
mod_BYM2$summary.hyperpar
```
This tells us that almost all of the spatial variability is ecplained by the spatially structured component - strong spatial clustering. 
...
for(i in 1:317){
  RR_COVID[i] <- inla.emarginal(function(x) exp(x), 
        mod_BYM2$marginals.random$ID[[i]])
}
